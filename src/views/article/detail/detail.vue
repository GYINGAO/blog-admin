...
import { paramsToFormData, getFileNameFromUrl } from "@/utils/";
export default {
  name: "CreateArticle",
  components: { UploadPicture, Quill },
  data() {
    return {
      inputVisible: false,
      inputValue: "",
  ...
  methods: {
    async onSubmit() {
      ...
      const res = await articleAPI.update(formData);
    },
    ...
    async getDetail() {
      const { data } = await articleAPI.show(this.$route.params.id);
      let {
        tags,
        cover,
        title,
      ...
      const cover_arr = [];
      tags.forEach(item => {
        tag_arr.push(item.name);
      });
      cover = cover ? cover.split(",") : [];
      cover.forEach(item => {
        cover_arr.push({ name: getFileNameFromUrl(item), url: item });
      });
      this.article = {
        title: title,
        summary: summary,
